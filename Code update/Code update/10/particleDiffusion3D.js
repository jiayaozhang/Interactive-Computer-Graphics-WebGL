...
window.onload = function init() {
    ...
    for(var i = 0; i<numPoints; i++)
         vertices[4+i] = vec3(2.0*Math.random()-1.0, 2.0*Math.random()-1.0, 2.0*Math.random()-1.0);
    ...
    gl.bufferData(gl.ARRAY_BUFFER, 64+12*numPoints, gl.STATIC_DRAW);
    gl.bufferSubData(gl.ARRAY_BUFFER, 0, flatten(vertices));
    gl.bufferSubData(gl.ARRAY_BUFFER, 32+12*numPoints, flatten(texCoord));
    ...
    gl.vertexAttribPointer( texCoordLoc, 2, gl.FLOAT, false, 0,32+12*numPoints );
...
var render = function(){
   ...
   // render to texture
    ...
    gl.useProgram(program1);
    gl.vertexAttribPointer( texLoc, 2, gl.FLOAT, false, 0, 32+12*numPoints);
